...
class VisionTransformer(nn.Module):
    def __init__(
            self, input_shape=[1, 1120], patch_size=16, in_chans=1, num_classes=256, num_features=108,
...
class TwoStreamTransformer(nn.Module):
    def __init__(self,  num_class,**kwargs):
        super().__init__()
        self.oneDvit = VisionTransformer()
        self.shijue = ViT(image_size = 64,
        patch_size = 16,
        num_classes = 256,
        dim = 32,
        depth = 12,
    ...
    def forward(self, inputs,inputs1):
        out1 = self.oneDvit(inputs)
        out2 = self.shijue(inputs1)
        concat=out1+out2
